...
def postgres_conn_1():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            logging.info(f"Connected to PostgreSQL (sql4) with client_encoding={encoding}")
            assert encoding.upper() == 'UTF8', f"Expected UTF8 encoding, got {encoding}"
    except UnicodeDecodeError as e:
        pytest.skip(f"Skipping DB tests due to psycopg2 UnicodeDecodeError: {e}")
    except Exception:
        raise
    yield conn
    conn.close()
...
def postgres_conn_2():
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("SHOW client_encoding;")
            encoding = cur.fetchone()[0]
            logging.info(f"Connected to PostgreSQL (sql7) with client_encoding={encoding}")
            assert encoding.upper() == 'UTF8', f"Expected UTF8 encoding, got {encoding}"
    except UnicodeDecodeError as e:
        pytest.skip(f"Skipping DB tests due to psycopg2 UnicodeDecodeError: {e}")
    except Exception:
...
class TestLifeCycleCellE2E:
    ...
    def test_application_service_crud(self, postgres_conn_1):
        ...
        app_data = {"name": "Test App", "description": "A test application.", "projectId": 1, "freelancerId": 1}
        app = client.create_application(app_data)
        logging.info(f"Created application: {app}")
    ...
    def test_application_service_validation(self):
        ...
        with pytest.raises(requests.exceptions.HTTPError) as excinfo:
            client.create_application({"name": ""})
    ...
    def test_full_e2e_flow(self, postgres_conn_2):
        ...
        logging.info("Starting full E2E flow test...")

        app_data = {"name": "E2E Flow App", "description": "Testing full flow.", "projectId": 1, "freelancerId": 1}
        app = app_client.create_application(app_data)
        logging.info(f"Application created: {app}")

        with postgres_conn_2.cursor() as cur:
            cur.execute("SHOW client_encoding;")
            encoding = cur.fetchone()[0]
            logging.info(f"Notifications DB client_encoding={encoding}")

        cursor = postgres_conn_2.cursor()
        cursor.execute("SELECT COUNT(*) FROM notifications")
        count_before = cursor.fetchone()[0]
        logging.info(f"Notifications before waiting: {count_before}")

        logging.info("Waiting 15 seconds for async notification process...")
        time.sleep(15)
        ...
        logging.info(f"Notifications after waiting: {count_after}")

        assert notification is not None, "Notification was not created after application creation"
        logging.info("Notification successfully found in database.")
        cursor.close()
